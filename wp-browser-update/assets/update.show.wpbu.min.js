// WPBU CSP adapter for browser-update.org update.show.min.js.
// Upstream: https://browser-update.org/update.show.min.js.
// Upstream SHA-256 on 2026-05-03: 9976811f7fd2ad3a89cc3f3450f9655938c39f3869de8000e50030e10a502747.
// Changes: moves generated styles to update.show.wpbu.css, uses classes for CSP compatibility, and defaults update links away from browser-update.org. */
"use strict";var $buo_show=function(){var op=window._buorgres;var bb=$bu_getBrowser();var urls={c:"https://chrome.google.com/",f:"https://firefox.com/",s:"https://support.apple.com/102665",e:"https://microsoft.com/edge",i:"https://blogs.windows.com/windowsexperience/2022/06/15/internet-explorer-11-has-retired-and-is-officially-out-of-support-what-you-need-to-know/",ios:"https://support.apple.com/ios",samsung:"https://www.samsung.com/apps/samsung-internet/",o:"https://opera.com/",o_a:"https://opera.com/browsers/opera-for-android",e_a:"https://microsoft.com/edge",y:"https://browser.yandex.com/",v:"https://vivaldi.com/",uc:"https://www.ucweb.com/",a:"https://www.android.com/"};var burl=op.burl||"";if(!op.url){op.url=burl?burl+((op.l&&(op.l+"/"))||"")+"update-browser.html"+(op.test?"?force_outdated=true":"")+"#"+op.jsv+":"+op.pageurl:urls[bb.n]||"https://browsehappy.com/";}
op.url_permanent_hide=op.url_permanent_hide||"#";function busprintf(){var args=arguments;var data=args[0];for(var k=1;k<args.length;++k){data=data.replace(/%s/,args[k]);}
return data;}
var t={},ta;t.en={'msg':'Your web browser ({brow_name}) is out of date.','msgmore':'Update your browser for more security, speed and the best experience on this site.','bupdate':'Update browser','bignore':'Ignore','remind':'You will be reminded in {days} days.','bnever':'Never show again','insecure':'Your web browser ({brow_name}) has a serious security vulnerability!'}
...
t=ta=t[op.llfull]||t[op.ll]||t.en;if(custom_text){if(typeof custom_text==='string')
t=ta=custom_text;else{for(var i in custom_text){ta[i]=custom_text[i]}}}
if(op.browser.is_insecure&&ta.insecure){ta.msg=ta.insecure}
if(ta.msg)
...
var tar="";if(op.newwindow)
tar=' target="_blank" rel="noopener"';var div=op.div=document.createElement("div");div.id="buorg";div.className="buorg buorg-"+(op.style||"top");if(ta.msg&&(op.ll=="ar"||op.ll=="he"||op.ll=="fa"))
div.className+=" buorg-rtl";if(!ta.msg&&t.indexOf&&t.indexOf("%s")!==-1){t=busprintf(t,bb.t,' id="buorgul" href="'+op.url+'"'+tar);div.className+=" buorg-legacy";div.innerHTML='<div>'+t+'<div id="buorgclose"><a id="buorga">&times;</a></div></div>';op.addmargin=true;}
else{if(op.style==="bottom"){}
else if(op.style==="corner"){}
else{op.addmargin=true;}
t=t.replace("{brow_name}",bb.t).replace("{up_but}",' id="buorgul" href="'+op.url+'"'+tar).replace("{ignore_but}",' id="buorgig" role="button" tabindex="0"');div.innerHTML='<div class="buorg-pad" role="status" aria-live="polite"><span class="buorg-icon"> </span>'+t+'</div>';}
op.text=t;if(op.container){op.container.appendChild(div);op.addmargin=false;}
else
document.body.insertBefore(div,document.body.firstChild);var updatebutton=document.getElementById("buorgul");if(updatebutton){updatebutton.onclick=function(e){div.onclick=null;op.onclick(op);if(op.noclose)
return
op.setCookie(op.reminderClosed);if(!op.noclose){div.className+=" buorg-hidden";if(op.addmargin&&op.shift_page_down!==false)
hm.className=hm.className.replace(/\s?buorg-shift-page-down/g,"");}};}
if(!custom_text){div.className+=" buorg-clickable";div.onclick=function(){if(op.newwindow)
window.open(op.url,"_blank");else
window.location.href=op.url;op.setCookie(op.reminderClosed);op.onclick(op);};}
if(op.addmargin&&op.shift_page_down!==false){var hm=document.getElementsByTagName("html")[0]||document.body;op.bodymt="";if((" "+hm.className+" ").indexOf(" buorg-shift-page-down ")<0)hm.className+=" buorg-shift-page-down";}
var ignorebutton=document.getElementById("buorga")||document.getElementById("buorgig");if(ignorebutton){ignorebutton.onclick=function(e){div.onclick=null;op.onclose(op);if(op.addmargin&&op.shift_page_down!==false)
hm.className=hm.className.replace(/\s?buorg-shift-page-down/g,"");op.setCookie(op.reminderClosed);if(!op.no_permanent_hide&&ta.bnever&&ta.remind){op.div.innerHTML='<div class="buorg-pad"><span class="buorg-moremsg">'+(op.reminderClosed>24?ta.remind.replace("{days}",Math.round(op.reminderClosed/24)):"")+'</span> <span class="buorg-buttons"><a id="buorgpermanent" role="button" tabindex="0" href="'+op.url_permanent_hide+'"'+tar+'>'+ta.bnever+'</a></span></div>';div.className="buorg buorg-fadeout";document.getElementById("buorgpermanent").onclick=function(e){op.setCookie(24*365);op.div.className+=" buorg-hidden";}
op.div.className+=" buorg-hidden";return false;}
op.div.className+=" buorg-hidden";return false;}
if(op.noclose||op.reminderClosed==0){ignorebutton.parentNode.removeChild(ignorebutton)}}
op.onshow(op);if(op.test&&!op.dont_show_debuginfo){var e=document.createElement("script");e.src=op.domain+"/update.test.wpbu.js";document.body.appendChild(e);}};$buo_show();
